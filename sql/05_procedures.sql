...
AS
    ...
    v_wrong_owner_count NUMBER;
BEGIN
    -- 1. Kiểm tra số lượng thú cưng muốn thêm phải hợp lệ
    IF p_pet_count IS NULL OR p_pet_count <= 0 THEN
        RAISE_APPLICATION_ERROR(-20050,'LỖI DỮ LIỆU: Số lượng thú cưng muốn thêm phải lớn hơn 0.');
    END IF;
    ...
    -- 3. Đếm số thú cưng hiện đã có trong phòng
    SELECT COUNT(*)
    INTO   v_existing_count
    FROM   booking_room_pet brp
    WHERE  brp.booking_room_id = p_booking_room_id;

    -- 4. Kiểm tra các thú cưng đã có trong phòng có cùng chủ với booking không
    SELECT COUNT(*)
    INTO v_wrong_owner_count
    FROM booking_room_pet brp
    JOIN pet p ON p.pet_id = brp.pet_id
    WHERE brp.booking_room_id = p_booking_room_id
      AND p.customer_id <> v_booking_owner;

    IF v_wrong_owner_count > 0 THEN
        RAISE_APPLICATION_ERROR(-20052, 'LỖI CHỦ SỞ HỮU: Trong phòng đã có thú cưng không thuộc cùng chủ với booking.');
    END IF;

    -- 5. Kiểm tra tổng số thú cưng sau khi thêm không vượt sức chứa
    IF v_existing_count + p_pet_count > v_max_pets THEN
        RAISE_APPLICATION_ERROR(-20053,'LỖI SỨC CHỨA: Tổng số thú cưng sau khi thêm là ' || (v_existing_count + p_pet_count) 
...
BEGIN
    ...
    FOR rec IN (
        ...
        v_found_standard := v_found_standard + 1;
    ...
    IF v_found_standard = 0 THEN
        RAISE_APPLICATION_ERROR(-20034, 'LỖI: Không tìm thấy định mức phù hợp cho thú cưng.');
    END IF;
...
CREATE OR REPLACE PROCEDURE sp_assign_pet_to_room (
    ...
    v_max_pets            type_room.max_pets%TYPE;
    v_booking_owner       customer.customer_id%TYPE;
    v_new_pet_owner       customer.customer_id%TYPE;
    v_wrong_owner_count   NUMBER;
BEGIN
    -- 1. Lấy sức chứa tối đa và chủ sở hữu của booking
    SELECT tr.max_pets, b.customer_id
    INTO   v_max_pets, v_booking_owner
    FROM   booking_room br
    JOIN   booking b    ON b.booking_id = br.booking_id
    JOIN   room r       ON r.room_id = br.room_id
    JOIN   type_room tr ON tr.type_room_id = r.type_room_id
    ...
    -- 2. Lấy chủ sở hữu của thú cưng mới
    SELECT p.customer_id
    INTO   v_new_pet_owner
    FROM   pet p
    WHERE  p.pet_id = p_pet_id;

    -- 3. Kiểm tra thú cưng mới phải thuộc khách hàng của booking
    IF v_new_pet_owner <> v_booking_owner THEN
        RAISE_APPLICATION_ERROR(-20044,'LỖI CHỦ SỞ HỮU: Thú cưng không thuộc khách hàng của booking, không thể gán vào phòng.');
    END IF;

    -- 4. Đếm số thú cưng hiện có trong phòng
    SELECT COUNT(*)
    INTO   v_current_pets
    FROM   booking_room_pet brp
    WHERE  brp.booking_room_id = p_booking_room_id;

    -- 5. Kiểm tra các thú cưng đang ở trong phòng có cùng chủ không
    SELECT COUNT(*)
    INTO   v_wrong_owner_count
    FROM   booking_room_pet brp
    JOIN   pet p ON p.pet_id = brp.pet_id
    ...
    IF v_wrong_owner_count > 0 THEN
        RAISE_APPLICATION_ERROR(-20045,'LỖI CHỦ SỞ HỮU: Trong phòng đã có thú cưng không thuộc cùng chủ với booking.');
    END IF;
    ...
    INSERT INTO booking_room_pet (booking_room_id,pet_id,assigned_at,note)
    VALUES (p_booking_room_id,p_pet_id,SYSTIMESTAMP,'Gán thú cưng vào phòng bằng procedure');

EXCEPTION
    WHEN DUP_VAL_ON_INDEX THEN
        RAISE_APPLICATION_ERROR(-20046,'LỖI TRÙNG DỮ LIỆU: Thú cưng này đã được gán vào phòng.');

    WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20047,'LỖI DỮ LIỆU: Không tìm thấy booking_room_id hoặc pet_id tương ứng.');
...
BEGIN
    ...
    -- NVL phòng trường hợp chưa có khoản nào thì coi như 0
    ...
    -- Điều kiện PAID: đã thanh toán đủ và dịch vụ/phòng đã hoàn thành
    ...
    ELSIF v_total_paid = v_grand_total 
        AND fn_is_order_ready_to_pay(v_order_id) THEN
        UPDATE orders
        SET status = 'PAID'
        WHERE order_id = v_order_id;
    END IF;
...
BEGIN
    -- 1. Lấy trạng thái hiện tại của booking và khóa dòng booking
    SELECT b.status
    ...
    -- 3. Khóa tất cả phòng thuộc booking và kiểm tra phòng phải AVAILABLE
    FOR rec IN (
        ...
        IF rec.status <> 'AVAILABLE' THEN
            RAISE_APPLICATION_ERROR(
                -20061,
                'LỖI CHECK-IN: Phòng ' || rec.room_id ||
                ' đang ở trạng thái ' || rec.status ||
                ', không thể thực hiện check-in.'
            );
        END IF;
    END LOOP;

    -- 4. Nếu booking chưa được gán phòng thì không cho check-in
    IF v_room_count = 0 THEN
        RAISE_APPLICATION_ERROR(
            -20063,
            'LỖI CHECK-IN: Booking chưa được gán phòng.'
        );
    END IF;

    -- 5. Chuyển trạng thái booking sang CHECKED_IN
    UPDATE booking
    SET    status            = 'CHECKED_IN',
           checkin_actual_at = SYSTIMESTAMP
    WHERE  booking_id = p_booking_id;

    -- 6. Chuyển tất cả phòng liên kết sang IN_USE
    UPDATE room r
    SET    r.status = 'IN_USE'
    WHERE  r.room_id IN (
        SELECT br.room_id
...
BEGIN
    ...
    FOR rec IN (
        ...
        AND booking_room_id = rec.booking_room_id;
