...
ALTER TABLE branch ADD (
    average_rating NUMBER(2,1) DEFAULT 0 NOT NULL,
    review_count NUMBER DEFAULT 0 NOT NULL
);

ALTER TABLE branch ADD CONSTRAINT ck_branch_avg_rating 
CHECK (average_rating >= 0 AND average_rating <= 5);

ALTER TABLE branch ADD CONSTRAINT ck_branch_review_count 
...
CREATE TABLE type_room (
    ...
    CONSTRAINT ck_type_room_min_weight 
    CHECK (min_weight_kg IS NULL OR min_weight_kg >= 0),
    CONSTRAINT ck_type_room_max_weight 
    CHECK (max_weight_kg IS NULL OR max_weight_kg > 0),
    CONSTRAINT ck_type_room_weight_range 
    CHECK (
...
CREATE TABLE room (
    ...
    CONSTRAINT uq_room_number_branch UNIQUE (room_number, branch_id),
...
CREATE TABLE booking (
    ...
    checkin_actual_at       TIMESTAMP(6) WITH TIME ZONE,
    checkout_actual_at      TIMESTAMP(6) WITH TIME ZONE,
    status                  NVARCHAR2(20) NOT NULL,
    ...
    CONSTRAINT ck_booking_time_actual CHECK (
        checkin_actual_at IS NULL 
        OR checkout_actual_at IS NULL
        OR checkin_actual_at < checkout_actual_at
    )
);
...
CREATE TABLE booking_services_pet (
    ...
    CONSTRAINT pk_bks_pet PRIMARY KEY (booking_service_id),
    CONSTRAINT fk_bks_pet_booking FOREIGN KEY (booking_id) REFERENCES booking(booking_id),
    CONSTRAINT fk_bks_pet_service FOREIGN KEY (service_id) REFERENCES services(service_id),
    CONSTRAINT fk_bks_pet_employee FOREIGN KEY (employee_id) REFERENCES employee(employee_id),
    CONSTRAINT fk_bks_pet_pet FOREIGN KEY (pet_id) REFERENCES pet(pet_id),
    CONSTRAINT ck_bks_pet_status CHECK (status IN ('PENDING','SCHEDULED','IN_PROGRESS','DONE','CANCELLED'))
);
...
CREATE TABLE coupon (
    ...
    CONSTRAINT fk_coupon_employee FOREIGN KEY (employee_id) REFERENCES employee(employee_id),
    CONSTRAINT ck_coupon_type CHECK (discount_type IN ('FIXED', 'PERCENT')),
    CONSTRAINT ck_coupon_value CHECK (discount_value > 0),
    CONSTRAINT ck_coupon_percent CHECK (discount_type <> 'PERCENT' OR discount_value <= 100),
    CONSTRAINT ck_coupon_min_order CHECK (min_order_value >= 0),
    CONSTRAINT ck_coupon_max_uses CHECK (max_uses IS NULL OR max_uses > 0),
    CONSTRAINT ck_coupon_used_count CHECK (used_count >= 0),
    CONSTRAINT ck_coupon_date_range CHECK (expired_at > effective_from),
    CONSTRAINT ck_coupon_active CHECK (is_active IN (0, 1))
);

CREATE INDEX idx_coupon_active_dates ON coupon (is_active, effective_from, expired_at);
-- =========================================================
-- 28. BOOKING_COUPON_LOG
-- =========================================================
CREATE TABLE booking_coupon_log (
    booking_id      VARCHAR2(10) NOT NULL,
    coupon_id       VARCHAR2(10) NOT NULL,
    applied_at      TIMESTAMP(6) WITH TIME ZONE DEFAULT SYSTIMESTAMP NOT NULL,
    note            CLOB,
    CONSTRAINT pk_booking_coupon_log PRIMARY KEY (booking_id, coupon_id),
    CONSTRAINT fk_bcl_booking FOREIGN KEY (booking_id) REFERENCES booking(booking_id),
    CONSTRAINT fk_bcl_coupon FOREIGN KEY (coupon_id) REFERENCES coupon(coupon_id)
);
