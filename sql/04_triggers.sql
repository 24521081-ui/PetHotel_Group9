...
BEGIN
    SELECT COUNT(*) INTO v_current_pets 
    FROM booking_room_pet 
    WHERE booking_room_id = :NEW.booking_room_id
        AND PET_id <> :NEW.pet_id;

    SELECT tr.max_pets INTO v_max_pets 
    FROM booking_room br JOIN room r ON r.room_id = br.room_id 
    ...
    IF v_current_pets >= v_max_pets THEN
        RAISE_APPLICATION_ERROR(-20041, 'LỖI SỨC CHỨA.'); 
    END IF;
    ...
    IF v_current_pets > 0 THEN
        SELECT customer_id INTO v_owner_new 
        FROM pet 
        WHERE pet_id = :NEW.pet_id;

        SELECT p.customer_id INTO v_owner_existing 
        FROM booking_room_pet brp JOIN pet p ON p.pet_id = brp.pet_id 
        WHERE brp.booking_room_id = :NEW.booking_room_id AND ROWNUM = 1;
        
        IF v_owner_new <> v_owner_existing THEN 
            RAISE_APPLICATION_ERROR(-20043, 'LỖI CHỦ SỞ HỮU.');
        END IF;
    END IF;
END;
/
...
BEGIN
    IF :NEW.status = 'SUCCESS' AND :NEW.paid_at IS NULL THEN 
        RAISE_APPLICATION_ERROR(-20062, 'LỖI: SUCCESS phải có paid_at.'); 
    END IF;

    IF :NEW.status IN ('PENDING', 'FAILED', 'REFUNDED') AND :NEW.paid_at IS NOT NULL THEN 
        RAISE_APPLICATION_ERROR(-20063, 'LỖI: Trạng thái '||:NEW.status||' phải NULL paid_at.'); 
    END IF;
END;
/
...
BEGIN
    ...
    IF v_eb <> v_bb THEN 
        RAISE_APPLICATION_ERROR(-20021, 'LỖI CHI NHÁNH.'); 
    END IF;
END;
/
...
BEGIN
    ...
    ELSIF UPDATING THEN
        IF :OLD.status IN ('PENDING', 'SCHEDULED', 'CANCELLED') AND :NEW.status IN ('IN_PROGRESS', 'DONE') THEN
            sp_validate_and_execute_stock(:NEW.booking_id, :NEW.service_id, :NEW.pet_id);

        ELSIF :OLD.status IN ('IN_PROGRESS', 'DONE') AND :NEW.status = 'CANCELLED' THEN
            sp_refund_service_stock(:OLD.booking_id, :OLD.service_id, :OLD.pet_id);

        ELSIF :OLD.status IN ('IN_PROGRESS', 'DONE') AND :NEW.status IN ('IN_PROGRESS', 'DONE')
              AND (:OLD.service_id <> :NEW.service_id OR :OLD.pet_id <> :NEW.pet_id OR :OLD.booking_id <> :NEW.booking_id) THEN
            sp_refund_service_stock(:OLD.booking_id, :OLD.service_id, :OLD.pet_id);
            sp_validate_and_execute_stock(:NEW.booking_id,:NEW.service_id,:NEW.pet_id);
        END IF;
...
BEGIN
    ...
    ELSE
        v_order_id := :NEW.order_id;
        v_delta := :NEW.line_total - :OLD.line_total;
    END IF;

    v_deposit := fn_calc_deposit_applied(v_order_id);

    UPDATE orders
    SET subtotal = subtotal + v_delta,
        grand_total = GREATEST(subtotal + v_delta - v_deposit, 0)
...
BEGIN
    ...
    WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20090, 'Không tìm thấy booking để áp dụng tiền cọc.');
END;
/
...
DECLARE
    v_d NUMBER;
BEGIN
    SELECT NVL(deposit_amount, 0) INTO v_d 
    FROM booking 
...
BEGIN
    IF :NEW.status = 'PAID' AND :OLD.status <> 'PAID' THEN
        IF NOT fn_is_order_ready_to_pay(:NEW.order_id) THEN
            RAISE_APPLICATION_ERROR(-20080,'LỖI: Không thể chuyển hóa đơn sang PAID khi dịch vụ/phòng chưa hoàn tất.');
        END IF;
    END IF;
END;
/
